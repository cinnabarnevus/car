...
void SPI1_Init(void)	
{
	...
	//用于SPI1的MISO(主输入从输出)管脚配置，但是spi作为lcd的控制器时，MISO管脚不需要使用，因此该段配置在本项目中未用到
	...
	SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;//工作模式1，时钟极性为0，时钟相位为0
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;//软件管理NSS信号
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;//从高位开始发送
